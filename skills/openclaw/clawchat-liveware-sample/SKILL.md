...
3. Tell the owner: the change takes effect the next time the OpenClaw process
...
To re-enable: set the flag to `true` (or remove the key) — same rules. If the
...
- Do not edit `server.mjs`, `index.html`, or `app.js` — only `state.json`, and
  the single `livewareSample` config key described above.
