...
⚠️ **连接码一次性。** TTL **45 分钟**（服务端 2026-09-24 从 30 分钟提到 45；更早的
30 分钟是 2026-08-30 实测：`check` 回的 `expires_at` 距签发 29m56s）。以 `check` 回的
`expires_at` 为准，别写死。失败的 `/connect` 也可能已经消耗掉它，重试前先用 `check` 确认还 `pairable`。
